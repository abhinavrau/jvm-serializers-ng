...
public class InfoCmd implements Callable<Void> {
  ...
  private static final String tableOptionHelp = " Hint: Combine with -t (--table) option to see extended metadata";
  @CommandLine.ParentCommand
  private Cli parent; // picocli injects reference to parent command

  @CommandLine.Option(names = {"-a", "--all"}, description =
      "List all the available serializers and relevant metadata." + tableOptionHelp)
  private boolean listAll = false;

  @CommandLine.Option(names = {"-n",
      "--names"}, description = "List only the names of supported serializer names with no related metadata i.e. json, xml, protobuf etc.")
  private boolean names = false;

  @CommandLine.Option(names = {"-b", "--binary"}, description =
      "List binary names serializers and relevant metadata." + tableOptionHelp)
  private boolean binary = false;

  @CommandLine.Option(names = {"-j", "--json"}, description =
      "List json names serializers and relevant metadata." + tableOptionHelp)
  private boolean json = false;

  @CommandLine.Option(names = {"-x", "--xml"}, description =
      "List xml names serializers relevant metadata." + tableOptionHelp)
  private boolean xml = false;

  @CommandLine.Option(names = {"-e", "--text"}, description =
      "List text names serializers and relevant metadata i.e. json & xml (same as calling with -jx)."
          + tableOptionHelp)
  private boolean text = false;

  @CommandLine.Option(names = {"-t",
      "--table"}, description = "Outputs all metadata in a more readable ascii table")
  private boolean tableFormat = false;

  @CommandLine.Option(names = {"-f", "--find"}, arity = "1..*", description =
      "Finds serializers with the one or more <keywords> associated with it in it's metadata."
          + tableOptionHelp)
  ...
  public Void call() {
    ...
    ImmutableTable<String, SerializerProperties, TestGroup.Entry<MediaContent, Object>> table
        = ImmutableTable.copyOf(groups.media.getTable());
  ...
  private void printAsciiTable(NavigableSet<String> output) {
    ...
    if (names) {
      ...
      header = asciiTable
          .addRow("#", "Name", "Format", "ApiStyle", "Value Type", "Mode", "Features",
              "Optimization", "URL");
    }
    int index = 0;
    for (String s : output) {
      ...
      if (names) {
        ...
        asciiTable
            .addRow(++index, splitter.next(), splitter.next(), splitter.next(), splitter.next(),
                splitter.next(), splitter.next(), splitter.next(), splitter.next());
      }
    }
    asciiTable.addRule();
  ...
  private void filterSerializers(NavigableSet<String> output,
    ...
    for (Table.Cell<String, SerializerProperties, TestGroup.Entry<MediaContent, Object>> entry : table
        .cellSet()) {
