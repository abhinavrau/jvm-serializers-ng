...
abstract class BenchmarkBase {
  ...
  /**
   * Number of milliseconds to warm up for each operation type for each serializer. Let's start with
   * 3 seconds.
   */
  ...
  protected void findParameters(String[] args, Params params) {
    ...
    for (String arg : args) {
      ...
      if (arg.startsWith("--")) {
        ...
        System.err.println(
            "Expecting only one non-option argument (<data-file> = \"" + params.dataFileName
                + "\").");
      ...
      if (option.equals("include")) {
        ...
        if (value != null) {
          System.err
              .println("The \"skip-pre-warmup\" option does not take a value: \"" + arg + "\"");
          System.exit(1);
        }
        ...
        System.out.println(
            "  -skip-pre-warmup      (don't warm all serializers before the first measurement)");
  ...
  private void validateDataFile(Params params) {
    ...
    InputStream dataFile = this.getClass().getClassLoader()
        .getResourceAsStream(params.dataFileName);
    if (dataFile == null) {
    ...
    if (parts.length < 3) {
      System.err.println("Data file \"" + params.dataFileName
          + "\" should be of the form \"<type>.<name>.<extension>\"");
      System.exit(1);
    }
  ...
  public void checkForCorrectness(MediaContentTestGroup groups, String dataFileName)
      throws Exception {
    Params params = new Params();
  ...
  protected void runTests(MediaContentTestGroup groups, Params params,
    ...
    PrintWriter errorsPW = new PrintWriter(errors);
    try {
      EnumMap<measurements, Map<String, Double>> values = runMeasurements(errorsPW, params,
          matchingEntries, testData,
          testCreate,
          testSerialize,
          testDeserialize
      );
  ...
  protected abstract Object convertTestData(TestGroup.Entry<?, Object> loader, Params params,
      byte[] data)
      throws Exception;
  ...
  protected Object loadTestData(TestGroup<?> bootstrapGroup, Params params) {
    ...
    byte[] fileBytes;
    try {
      fileBytes = IOUtils.toByteArray(this.getClass().getClassLoader()
          .getResourceAsStream(params.dataFileName)); // Load entire file into a byte array.
    } catch (Exception ex) {
  ...
  /**
   * Method called to both load in test data and figure out which tests should actually be run, from
   * all available test cases.
   */
  protected Iterable<TestGroup.Entry<Object, Object>> findApplicableTests(
      MediaContentTestGroup groups, Params params,
      TestGroup<?> bootstrapGroup) {
    @SuppressWarnings("unchecked")
    TestGroup<Object> group_ = (TestGroup<Object>) bootstrapGroup;
    Set<String> matched = new HashSet<String>();

    Iterable<TestGroup.Entry<Object, Object>> available = group_.getTable().values();
    ...
    for (String s : unmatched) {
      System.err
          .println("Warning: there is no implementation name matching the pattern \"" + s + "\"");
    }
  ...
  protected <J> EnumMap<measurements, Map<String, Double>> runMeasurements(PrintWriter errors,
    ...
    if (params.prewarm) {
      ...
      for (TestGroup.Entry<J, Object> entry : groups) {
        TestCaseRunner<J> runner = new TestCaseRunner<J>(entry.transformer, entry.serializer,
            value);
        String name = entry.serializer.getName();
    ...
    System.out.printf("%-34s %6s %7s %7s %7s %6s %5s\n",
        ...
        "size",
        "+dfl");
    EnumMap<measurements, Map<String, Double>> values = new EnumMap<measurements, Map<String, Double>>(
        measurements.class);
    ...
    for (TestGroup.Entry<J, Object> entry : groups) {
      ...
      try {
        ...
        double timeCreate = runner
            .runWithTimeMeasurement(params.testRunMillis / 3, testCreate, params.iterations);

        warmTest(runner, params.warmupTime, testSerialize);

        doGc();
        double timeSerialize = runner
            .runWithTimeMeasurement(params.testRunMillis, testSerialize, params.iterations);

        doGc();
        double timeDeserialize = runner
            .runWithTimeMeasurement(params.testRunMillis, testDeserialize, params.iterations);
  ...
  protected abstract <J> byte[] serializeForSize(Transformer<J, Object> tranformer,
  ...
  protected <J> void warmTest(TestCaseRunner<J> runner, long warmupTime, TestCase test)
      throws Exception {
   ...
   * serializer, deserialize; compare objects after steps 1 and 3).
   */
  protected abstract <J> void checkCorrectness(PrintWriter errors,
      Transformer<J, Object> transformer,
      Serializer<Object> serializer, J value)
      throws Exception;
  ...
  protected static void printImage(Map<String, Double> map, measurements m) {
    ...
    System.out.println("<img src='https://chart.googleapis.com/chart?chtt="
        ...
        + "&chf=c||lg||0||FFFFFF||1||76A4FB||0|bg||s||EFEFEF&chs=" + width + "x" + height
        + "&chd=t:"
        + valSb.toString().substring(0, valSb.length() - 1)
        + "&chds=0," + scale
        + "&chxt=y"
        + "&chxl=0:|" + names.substring(0, names.length() - 1)
        + "&chm=N *f*,000000,0,-1,10&lklk&chdlp=t&chco=660000|660033|660066|660099|6600CC|6600FF|663300|663333|663366|663399|6633CC|6633FF|666600|666633|666666&cht=bhg&chbh="
        + barThickness + ",0," + barSpacing + "&nonsense=aaa.png'/>");
