...
/**
 * "Hand-written" version of Jackson-based codec. Not optimized for compactness, but code is
 * relatively simple monkey code even if bit verbose.
 */
public class JacksonJsonManual extends BaseJacksonDriver<MediaContent> {

  protected final static SerializedString FIELD_IMAGES = new SerializedString(
      FULL_FIELD_NAME_IMAGES);

  protected final static SerializedString FIELD_MEDIA = new SerializedString(FULL_FIELD_NAME_MEDIA);
  protected final static SerializedString FIELD_PLAYER = new SerializedString(
  ...
  protected final static SerializedString FIELD_HEIGHT = new SerializedString(
      FULL_FIELD_NAME_HEIGHT);
  protected final static SerializedString FIELD_FORMAT = new SerializedString(
      FULL_FIELD_NAME_FORMAT);
  protected final static SerializedString FIELD_DURATION = new SerializedString(
      FULL_FIELD_NAME_DURATION);
  protected final static SerializedString FIELD_SIZE = new SerializedString(FULL_FIELD_NAME_SIZE);
  protected final static SerializedString FIELD_BITRATE = new SerializedString(
      FULL_FIELD_NAME_BITRATE);
  protected final static SerializedString FIELD_COPYRIGHT = new SerializedString(
      FULL_FIELD_NAME_COPYRIGHT);
  protected final static SerializedString FIELD_PERSONS = new SerializedString(
      FULL_FIELD_NAME_PERSONS);
  ...
  protected MediaContent readMediaContent(JsonParser parser) throws IOException {
    ...
    if (mc.media == null) {
      throw new IllegalStateException("Missing field: " + FIELD_MEDIA);
    }
    if (mc.images == null) {
  ...
  private Media readMedia(JsonParser parser) throws IOException {
    ...
    if (media.uri == null) {
      throw new IllegalStateException("Missing field: " + FIELD_URI);
    }
    if (!haveWidth) {
      throw new IllegalStateException("Missing field: " + FIELD_WIDTH);
    }
    if (!haveHeight) {
      throw new IllegalStateException("Missing field: " + FIELD_HEIGHT);
    }
    if (media.format == null) {
      throw new IllegalStateException("Missing field: " + FIELD_FORMAT);
    }
    if (!haveDuration) {
      throw new IllegalStateException("Missing field: " + FIELD_DURATION);
    }
    if (!haveSize) {
      throw new IllegalStateException("Missing field: " + FIELD_SIZE);
    }
    if (media.persons == null) {
      media.persons = new ArrayList<String>();
    }
    if (media.player == null) {
      throw new IllegalStateException("Missing field: " + FIELD_PLAYER);
    }
