...
public class SerializerProperties {
  ...
  /**
   * The type of Value type the serializer supports that can be passed or used with a given
   * serializer API
   */
  ...
  public enum APIStyle {
    ...
    /**
     * Uses Byte Code generation at runtime Usually common with annotations that to mark fields to
     * be serialized.
     */
  ...
  @Override
  public int hashCode() {
    return Objects.hash(name, format, mode, valueType, apiStyle, features, projectUrl, typeMapper,
        optimizedDescription, relatedSerializer, locSerializer, locTransformer);
  }
}
