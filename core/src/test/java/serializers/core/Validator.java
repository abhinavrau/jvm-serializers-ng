...
public class Validator<S, T> {
  ...
  public void checkForCorrectness(final MediaContentTestGroup groups, final String dataPath)
      throws Exception {
    ...
    if (!dataPath.endsWith(".json")) {
      ...
      for (String filePath : jsonFiles) {
        ...
        checkForCorrectness(groups, this.getClass().getClassLoader().getResourceAsStream(filePath),
            filePath);

      }

    } else {
      checkForCorrectness(groups, this.getClass().getClassLoader().getResourceAsStream(dataPath),
          dataPath);
    }
  }
  ...
  protected void checkForCorrectness(final MediaContentTestGroup groups,
    ...
    for (Table.Cell<String, SerializerProperties, TestGroup.Entry<MediaContent, Object>> entry : entries) {
      ...
      logger.info("Testing serializer: {} with file: {}", entry.getColumnKey().getShortName(),
          dataFileName);
  ...
  private void checkCorrectness(final Serializer<T> serializer, final Transformer<S, T> transformer,
      final S reference) throws Exception {
    ...
    ReflectionAssert
        .assertLenientEquals("De-serialized and transformed object does not match reference object",
            reference, reversed);
