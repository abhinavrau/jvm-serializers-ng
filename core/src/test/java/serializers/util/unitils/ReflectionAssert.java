...
import static serializers.util.unitils.ReflectionComparatorFactory.createRefectionComparator;
...
/**
 * A class for asserting that 2 objects/collections are equal by comparing properties and fields of
 * the objects/collections using reflection.
 * <p/>
 * The (combination of) comparator modes specify how strict the comparison must be:<ul>
 * <li>ignore defaults: compare only arguments (and inner values) that have a non default value (eg
 * null) as expected value</li>
 * <li>lenient dates: do not compare actual date values, just that they both have a value or
 * not</li>
 * <li>lenient order: order is not important when comparing collections or arrays</li>
 * </ul>
 * <p/>
 * There are 2 versions of each method: a lenient and a reflection version. With the ref versions
 * you can set the comparator modes explicitly (note: no modes means strict comparison). The len
 * versions are the same as the ref versions but have lenient order and ignore defaults set by
 * default.
 * <p/>
 * The name assert..ReflectionEquals is chosen instead of assert..Equals so it can be added as a
 * static import without naming collisions.
 *
 * @author Tim Ducheyne
 * @author Filip Neven
...
public class ReflectionAssert {
   ...
   * <p/>
   * This is identical to assertReflectionEquals with lenient order and ignore defaults set as
   * comparator modes.
   *
   ...
   * <p/>
   * This is identical to assertPropertyReflectionEquals with lenient order and ignore defaults set
   * as comparator modes.
   *
  ...
  public static void assertPropertyLenientEquals(String message, String propertyName,
      Object expectedPropertyValue, Object actualObject) throws AssertionFailedError {
    assertPropertyReflectionEquals(message, propertyName, expectedPropertyValue, actualObject,
        LENIENT_ORDER, IGNORE_DEFAULTS);
  }
  ...
  public static void assertPropertyReflectionEquals(String message, String propertyName,
    ...
    String formattedMessage = formatMessage(message,
        "Incorrect value for property: " + propertyName);
   ...
   * <p/>
   * Example:  assertPropertyEquals("id", myIdCollection, myObjectCollection) checks whether all
   * values of the id field of the myObjectCollection elements matches the values in the
   * myIdCollection
   * <p/>
   * Bean notation can be used to specify inner properties. Eg myArray[2].innerValue.
   * assertReflectionEquals is used to check whether both values are equal.
  ...
  public static void assertPropertyLenientEquals(String propertyName,
      Collection<?> expectedPropertyValues, Collection<?> actualObjects)
      throws AssertionFailedError {
  ...
  public static void assertPropertyReflectionEquals(String propertyName,
      Collection<?> expectedPropertyValues, Collection<?> actualObjects,
      ReflectionComparatorMode... modes) throws AssertionFailedError {
    assertPropertyReflectionEquals(null, propertyName, expectedPropertyValues, actualObjects,
        modes);
  }
  ...
  public static void assertPropertyLenientEquals(String message, String propertyName,
      Collection<?> expectedPropertyValues, Collection<?> actualObjects)
      throws AssertionFailedError {
    assertPropertyReflectionEquals(message, propertyName, expectedPropertyValues, actualObjects,
        LENIENT_ORDER, IGNORE_DEFAULTS);
  }

  /**
   * All fields are checked for null values (except the static ones). Private fields are also
   * checked. This is NOT recursive, only the values of the first object will be checked. An
   * assertion error will be thrown when a property is null.
   *
   * @param message a message for when the assertion fails
  ...
  public static void assertPropertyReflectionEquals(String message, String propertyName,
    ...
    Collection<?> actualPropertyValues = CollectionUtils
        .collect(actualObjects, new OgnlTransformer(propertyName));
  ...
  /**
   * Evaluates the given OGNL expression, and returns the corresponding property value from the
   * given object.
   *
  ...
  protected static Object getProperty(Object object, String ognlExpression) {
    try {
      ...
      throw new UnitilsException(
          "Failed to get property value using OGNL expression " + ognlExpression, e);
    }
  }

  /**
   * All fields that have a getter with the same name will be checked by an assertNotNull. Other
  ...
  /**
   * A commons collections transformer that takes an object and returns the value of the property
   * that is specified by the given ognl expression.
   */
  protected static class OgnlTransformer implements Transformer {
    ...
    /**
     * Transforms the given object in the value of the property that is specified by the ognl
     * expression.
     *
