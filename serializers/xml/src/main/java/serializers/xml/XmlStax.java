...
/**
 * Codec that works with standard full Stax implementations, where we have Stax input and output
 * factories.
 */
public class XmlStax {

  /**
   * Since XML streams must still have a single root, we'll use this as the tag
   */
  ...
  public static void register(MediaContentTestGroup groups, boolean woodstox, boolean aalto,
      boolean fastinfoset) {
    ...
    if (woodstox) {
      ...
      groups.media
          .add(JavaBuiltIn.mediaTransformer, new StaxMediaSerializer(properties, HANDLERS[0]));
    ...
    if (aalto) {
      ...
      groups.media
          .add(JavaBuiltIn.mediaTransformer, new StaxMediaSerializer(properties, HANDLERS[1]));
    }
    if (fastinfoset) {
      SerializerProperties properties = builder
          .name("fastinfoset")
      ...
      groups.media
          .add(JavaBuiltIn.mediaTransformer, new StaxMediaSerializer(properties, HANDLERS[2]));
    }
  }
