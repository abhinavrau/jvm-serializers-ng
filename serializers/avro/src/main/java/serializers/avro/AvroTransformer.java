...
/**
 * Transformer is needed when we use Avro-generated Java classes, to convert between these and POJOs
 * for verification purposes. This transformation is NOT done during performance testing, as it
 * could be viewed as unfair overhead for use cases where generated class is used instead of
 * equivalent POJO.
 */
public class AvroTransformer extends MediaTransformer<MediaContent> {
  ...
  private Media forwardMedia(data.media.Media media) {
    ...
    GenericArray<String> persons = new GenericData.Array<String>(media.persons.size(),
        Avro.Media.sPersons);
