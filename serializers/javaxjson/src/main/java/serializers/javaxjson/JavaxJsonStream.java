 ...
 * <p>
 * Code is based of JacksonJsonManual; first copy/pasted then modified to fit javax.json classes.
 * There was a "fast" path version, but given that there's no optimization possible with the API (no
 * nextFieldName like there is in Jackson), the only overhead we have here is the mapping between
...
public class JavaxJsonStream extends Serializer<MediaContent> {
  ...
  private final void verifyCurrent(JsonParser parser, Event current, Event expToken)
      throws IOException {
    if (current != expToken) {
