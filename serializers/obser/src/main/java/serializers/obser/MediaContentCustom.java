...
public class MediaContentCustom {
  ...
  private static Media readMedia(ObjectInputStream in) throws IOException {
    ...
    return new Media(uri, title, width, height, format, duration, size, bitrate, hasBitrate,
  ...
  public static void writeMaybeInt(ObjectOutputStream out, boolean exists, int value)
      throws IOException {
    if (exists) {
      out.writeBoolean(true);
      out.writeInt(value);
