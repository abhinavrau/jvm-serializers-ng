...
public class JsonijJpath
{
  ...
  static class SemiManualSerializer extends Serializer<MediaContent>
  {
    ...
    static MediaContent readMediaContent(String mediaContentJsonInput) throws Exception
    {
      ...
      Value bitrate = JPath.parse("/serializers.media/bitrate").evaluate(json);
      if (bitrate != null && !bitrate.isNull())
      {
        media.bitrate = bitrate.getInt();
        media.hasBitrate = true;
      }
      Value copyright = JPath.parse("/serializers.media/copyright").evaluate(json);
      if (copyright != null && !copyright.isNull())
      {
        media.copyright = copyright.getString();
      }
      media.duration = JPath.parse("/serializers.media/duration").evaluate(json).getNumber().longValue();
      media.format = JPath.parse("/serializers.media/format").evaluate(json).getString();
      media.height = JPath.parse("/serializers.media/height").evaluate(json).getInt();
      JSON.Array<Value> personValues = (JSON.Array<Value>) JPath.parse("/serializers.media/persons").evaluate(json);
      ...
      media.player = Media.Player.valueOf(JPath.parse("/serializers.media/player").evaluate(json).getString());
      media.size = JPath.parse("/serializers.media/size").evaluate(json).getNumber().longValue();
      media.title = JPath.parse("/serializers.media/title").evaluate(json).getString();
      media.uri = JPath.parse("/serializers.media/uri").evaluate(json).getString();
      media.width = JPath.parse("/serializers.media/width").evaluate(json).getInt();
