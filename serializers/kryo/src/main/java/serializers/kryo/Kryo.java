...
public class Kryo {
  ...
  public static class DefaultSerializer<T> extends Serializer<T> {
    ...
    public DefaultSerializer(SerializerProperties properties, TypeHandler<T> handler,
        boolean shared) {
      super(properties);
  ...
  /**
   * This shows how to use hand written serialization code with Kryo, while still leveraging Kryo
   * for most of the work. A serializer for each class can be implemented, as it is here, or the
   * classes to be serialized can implement an interface and host their own serialization code
   * (similar to java.io.Externalizable).
   */
  ...
  public static final TypeHandler<MediaContent> MediaTypeHandler = new TypeHandler<MediaContent>(
      MediaContent.class) {
    ...
    public void optimize(com.esotericsoftware.kryo.Kryo kryo) {
      ...
      FieldSerializer mediaContentSerializer = (FieldSerializer) kryo
          .getSerializer(MediaContent.class);
  ...
  static class MediaContentSerializer extends com.esotericsoftware.kryo.Serializer<MediaContent> {
    ...
    public MediaContent read(com.esotericsoftware.kryo.Kryo kryo, Input input,
        Class<MediaContent> type) {
      final Media media = kryo.readObject(input, Media.class);
      @SuppressWarnings("unchecked") final List<Image> images = (List<Image>) kryo
          .readObject(input, ArrayList.class, _imagesSerializer);
  ...
  static class MediaSerializer extends com.esotericsoftware.kryo.Serializer<Media> {
    ...
    public Media read(com.esotericsoftware.kryo.Kryo kryo, Input input, Class<Media> type) {
      return new Media(input.readString(), input.readString(), input.readInt(true),
          input.readInt(true), input.readString(),
          input.readLong(true), input.readLong(true), input.readInt(true), input.readBoolean(),
          (List<String>) kryo.readObject(
              input, ArrayList.class, _personsSerializer),
  ...
  static class ImageSerializer extends com.esotericsoftware.kryo.Serializer<Image> {
    ...
    public Image read(com.esotericsoftware.kryo.Kryo kryo, Input input, Class<Image> type) {
      return new Image(input.readString(), input.readString(), input.readInt(true),
          input.readInt(true), kryo.readObject(
          input, Size.class));
    }
